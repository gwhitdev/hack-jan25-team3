...
function whatPageOpen() {
  ...
  const openPage = pages.filter(
    (page) => !page.classList.contains("d-none")
  )[0];
...
async function submitReportForm() {
  let location =
    String(incidentLocation.lat) + ", " + String(incidentLocation.lng);
  try {
...
async function submitReporterForm() {
  ...
  try {
    const response = await fetch(`${databaseURL}/reporters`, {
...
async function initMap() {
  ...
  locationButton.textContent = "Take me to my location";
  locationButton.id = 'map-location-button';
